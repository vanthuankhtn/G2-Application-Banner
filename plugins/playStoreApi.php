...
class PlayStoreApi
{
  ...
  function topPaidApps($start = 1)
  {
    ...
    if (isset($this_content[0])) {
      ...
      foreach ($break_all_items as $li) {
        ...
        if (isset($ratings_context[1])) {
          $ratings = $ratings_context[1];
        } else {
          $ratings = 'Not defined';
        }
  ...
  function listCategories()
  {
    ...
    if (isset($this_content[0])) {
      ...
      foreach ($initial_load as $filtering_elements) {
        ...
        if (!empty($heading_element)) {
          $categories_array[$heading_element] = '';
        }
        foreach ($break_all_items as $li) {
          phpQuery::newDocument($li);
          ...
          if (isset($category_id_context[4])) {
            $category_id = str_replace('?feature=category-nav', '', $category_id_context[4]);
          } else {
  ...
  function searchStore($search_query, $sort = 'Popularity', $price = 'All', $safe_search = 'Off', $start = 1)
  {
    ...
    if (in_array($sort, $sort_array)) {
      $sort_term = $sort_array[$sort];
    } else {
      $sort_term = 0;
    }
    if (in_array($price, $price_array)) {
      $price_term = $price_array[$price];
    } else {
      $price_term = 0;
    }
    if (in_array($safe_search, $safe_search_array)) {
      $safe_search_term = $safe_search_array[$safe_search];
    } else {
      $safe_search_term = 0;
    }
  ...
  function itemInfo($item_id, $language)
  {
    $page_url = 'https://play.google.com/store/apps/details?id=' . $item_id . '&hl=' . $language;
    $this_content = $this->get_fcontent($page_url);
    ...
    if (isset($this_content[0])) {
      ...
      $banner_image = pq('img.cover-image')->attr('src');
      ...
      $app_title = pq('.document-title > div')->html();
      ...
      $html_app_description = pq('div.inner-container div.details-wrapper div.details-section div.show-more-content > div')->html();
      ...
      $app_score = pq('div.details-section-contents div.rating-box > div.score-container > div.score')->html();
      ...
      /*$app_info['General'][] = (object) array('app_store_url' => $app_store_url,'banner_image' => $banner_image,'banner_icon' => $banner_icon,'app_title' => $app_title, 'app_author' => $app_author,'author_store_url' => $author_store_url,
        ...
        'whats_new_html' => $whats_new_html,'whats_new_text_plain' => $whats_new_text_plain, 'score' => $app_score
      );*/
      $app_info['General'][] = (object)array(
        'app_store_url' => $page_url, 'banner_image' => $banner_image,
        'banner_icon' => $banner_icon, 'app_title' => $app_title,
        'app_author' => $app_author, 'author_store_url' => $author_store_url,
        'app_price' => $app_price, 'html_app_description' => $html_app_description,
        'text_plain_app_description' => $text_plain_app_description,
        'score' => $app_score
      );
      return $app_info;
    } else {
  ...
  function relatedViewed($item_id)
  {
    ...
    if (isset($this_content[0])) {
      ...
      foreach ($break_all_items as $li) {
        ...
        if (isset($related_app_rating_context[1])) {
          $related_rating = $related_app_rating_context[1];
        } else {
          $related_rating = 'Not defined';
        }
